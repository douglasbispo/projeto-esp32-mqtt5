...
// Definição da URL do Broker MQTT
#define CONFIG_BROKER_URL "mqtt://broker.hivemq.com:1883"

// Definição do pino GPIO para o LED 
#define LED_GPIO_PIN GPIO_NUM_2 
// Topico MQTT que será usado para enviar comandos de ligar/desligar o LED.
...
static void mqtt5_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        // Subscrever o tópico específico para o controle do LED
        msg_id = esp_mqtt_client_subscribe(client, MQTT_TOPIC_LED, 0);
        ESP_LOGI(TAG, "Subscribed to LED topic %s, msg_id=%d", MQTT_TOPIC_LED, msg_id);
    ...
    case MQTT_EVENT_UNSUBSCRIBED:
        ...
        // Essa linha foi comentada porque o cliente MQTT estava se desconectando automaticamente
    ...
    case MQTT_EVENT_DATA:
        ...
        /*  Está é a lógica para controlar o LED da placa com base nas mensagens MQTT.
            Verifica se o tópico da mensagem recebida é igual ao tópico de controle do LED */
        if (strncmp(event->topic, MQTT_TOPIC_LED, event->topic_len) == 0) {
             // Verifica se o payload da mensagem tem apenas um caracter "0" ou "1"
            if (event->data_len == 1) {
                if (event->data[0] == '1') {
                    gpio_set_level(LED_GPIO_PIN, 1); // liga o LED
                    ESP_LOGI(TAG, "LED ON"); // verifica o monitor serial 
                } else if (event->data[0] == '0') {
                    gpio_set_level(LED_GPIO_PIN, 0); // desliga o LED
                    ESP_LOGI(TAG, "LED OFF");
                } else {
                    // Log para comandos inválidos
                    ESP_LOGW(TAG, "Received invalid LED command: %.*s", event->data_len, event->data);
                }
            } else {
                //Log para payloads com comprimento inesperado
                ESP_LOGW(TAG, "Received invalid LED data length: %d", event->data_len);
            }
        }
...
void app_main(void)
{
    ...
    // reseta o pino para um estado padrão
    gpio_reset_pin(LED_GPIO_PIN);
    // configura o pino como uma saída
    gpio_set_direction(LED_GPIO_PIN, GPIO_MODE_OUTPUT);
    // define o nível inicial do pino (desligado ou ligado) e garante que comece desligado com o valor 0
